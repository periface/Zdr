...
namespace Zdr.Tests
{
    public abstract class ZdrTestBase : AbpIntegratedTestBase<ZdrTestModule>
    {
        ...
        protected void UsingDbContext(Action<ZdrDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Action<ZdrDbContext> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<ZdrDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<ZdrDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ZdrDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ZdrDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Action<ZdrDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ZdrDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<ZdrDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<ZdrDbContext, Task<T>> func)
        {
            T result;
